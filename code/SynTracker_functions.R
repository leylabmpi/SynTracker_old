...
synteny_analysis<-function(inpath, gene_name, tmp_folder) { 
  print("starting first function - synteny analysis")
  fas<-inpath
...
synteny_scores<- function(synteny_object, metadata) {
  
  print("starting second function - synteny scores")
  ...
  # create and populate named lists (=dicitioanris) of sample_ID -> different metadata fields
  GroupA<-list() # Family (== MotherID) 
  GroupB<-list() # Birth country
  GroupC<-list() # Birth state 
  GroupD<-list() # Birth town 
  
  GroupA[as.character(metadata$Sample)]<-as.character(metadata$GroupA)
  GroupB[as.character(metadata$Sample)]<-as.character(metadata$GroupB)
  GroupC[as.character(metadata$Sample)]<-as.character(metadata$GroupC)
  GroupD[as.character(metadata$Sample)]<-as.character(metadata$GroupD)
  ...
  for (i in 2:ncol(synteny_object)) {
    for (j in 1:(i-1)) {
      ...
      GroupA1<-GroupA[sample1][[1]]
      GroupA2<-GroupA[sample2][[1]]
      GroupB1<-GroupB[sample1][[1]]
      GroupB2<-GroupB[sample2][[1]]
      GroupC1<-GroupC[sample1][[1]]
      GroupC2<-GroupC[sample2][[1]]
      GroupD1<-GroupD[sample1][[1]]
      GroupD2<-GroupD[sample2][[1]]
      is.same.GroupA<-GroupA1 == GroupA2 
      is.same.GroupB<-GroupB1 == GroupB2 
      is.same.GroupC<-GroupC1 == GroupC2 
      is.same.GroupD<-GroupD1 == GroupD2 
